...
pub fn synchronize_all(
    ...
    if catalog.profiles.is_empty() {
        let synchronized_at_epoch_seconds = epoch_seconds();
        ...
        let previous_count = store
            .load_snapshot()?
            .map_or(0, |value| value.targets.len());
        atomic_write(
            store.paths().ssh_config.as_path(),
            b"# Generated by WarpgateSH. Changes will be replaced automatically.\n",
        )?;
